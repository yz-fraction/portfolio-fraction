...
class Ion_auth_model extends CI_Model
{
	...
	public function register( $username , $password , $email , $additional_data = array() , $groups = array() )
	{
		...
		if ( isset( $result[0]['id'] ) )
		{
			...
			# Insert $additional_data
			$this->db->insert( $this->tables['meta'] , $additional_data );
		}
		else
		{
			return FALSE;
	...
	public function add_to_group( $group_id , $user_id = FALSE )
	{
		...
		return $this->db->insert($this->tables['users_groups'] ,
								 array( $this->join['groups']	=> (int)$group_id ,
	...
	**/
	public function delete_user( $id )
	{
		...
		$this->trigger_events(array('post_delete_user',
	...
	public function change_email( $user_email )
	{
		$result = $this->db->update($this->tables['users'] ,
									array('email'	=> $user_email ) ,
									array('id'		=> $this->ion_auth->current_UID() )
							);
	...
	public function password_check( $password )
	{
		...
		$password = $this->hash_password_db( $UID, $password );
		
		if ( ! $password )
		{
			$this->set_error( lang('Error_password') );
