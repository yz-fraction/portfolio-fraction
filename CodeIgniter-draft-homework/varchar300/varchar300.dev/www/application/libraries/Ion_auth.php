...
class Ion_auth
{
	...
	*/
	...
	public function get_user_name_surname( $UID )
	{
		$query = $this->db->
						select('name , surname')->
						from('meta')->
	...
	public function get_user_name_surname_by_message_ID( $message_ID )
	{
		$query = $this->db->
						select('user_id')->
						from('message')->
		...
		{
			return $this->ion_auth->get_user_name_surname( $result[0]['user_id'] );
		}
		else
		{
			return FALSE;
